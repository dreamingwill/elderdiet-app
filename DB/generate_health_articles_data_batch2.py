...
def load_image_urls():
    """加载第二批图片URL映射"""
    url_file = "image_urls_batch2.json"
    if os.path.exists(url_file):
        with open(url_file, 'r', encoding='utf-8') as f:
            return json.load(f)
    else:
        print(f"⚠️  图片URL文件不存在: {url_file}")
        print("   请先运行 upload_images_to_oss_batch2.py 上传图片")
        return {}
...
def generate_health_articles_data():
    ...
    # 读取第二批Excel数据
    excel_path = "health_article_content_2/list.xlsx"
    if not os.path.exists(excel_path):
        print(f"❌ Excel文件不存在: {excel_path}")
        return None
    
    print(f"📖 读取第二批Excel数据: {excel_path}")
    df = pd.read_excel(excel_path)
    
    # 加载第二批图片URL映射
    print(f"🖼️  加载第二批图片URL映射...")
    image_urls = load_image_urls()
    
    # 生成文章数据
    articles = []
    # 使用不同的起始日期，避免与第一批重复
    base_date = datetime(2025, 5, 1)  # 第二批从4月开始
    ...
    for index, row in df.iterrows():
        try:
            ...
            article = {
                ...
                "status": 1,  # 已发布
                "is_featured": 1 if index < 100 else 0,  # 前15篇设为推荐
                "is_carousel": 1 if index < 10 else 0,   # 前3篇设为轮播
                "carousel_order": index + 6 if index < 3 else 0,  # 轮播顺序从6开始，避免与第一批冲突
    ...
    # 保存为JSON文件
    output_file = "health_articles_data_batch2.json"
...
if __name__ == "__main__":
    ...
    if result:
        print(f"\n🎉 第二批数据生成完成！共生成 {len(result)} 篇文章数据")
    else:
        print(f"\n💥 第二批数据生成失败")
