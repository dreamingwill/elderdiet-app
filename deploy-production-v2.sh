...
# ElderDiet 生产环境部署脚本 V2
# 兼容 docker compose 和 docker-compose 命令
# 使用方法: ./deploy-production-v2.sh

set -e  # 遇到错误立即退出

echo "🚀 ElderDiet 生产环境部署脚本 V2"
echo "=================================="
...
detect_docker_compose() {
    ...
    if command -v docker-compose &> /dev/null; then
        ...
        log_success "使用 docker-compose 命令"
    elif docker compose version &> /dev/null; then
        DOCKER_COMPOSE_CMD="docker compose"
        log_success "使用 docker compose 命令"
    else
        log_error "Docker Compose 未安装或不可用"
        log_info "请先运行修复脚本: ./fix-docker-issues.sh"
        exit 1
    fi
}
...
build_and_start() {
    ...
    # 构建镜像（使用缓存以节省时间）
    $DOCKER_COMPOSE_CMD -f docker-compose.prod.yml build --parallel
